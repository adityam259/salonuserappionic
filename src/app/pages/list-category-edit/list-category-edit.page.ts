...
@Component({
  ...
  templateUrl: './list-category-edit.page.html',
  styleUrls: ['./list-category-edit.page.scss'],
})
export class ListCategoryEditPage implements OnInit {
  itemId: string;
  message: any;
  listOfHeaders: any;
  file: File;
  ...
  async getHeaderImageDetails(id) {
    ...
    this.requestServiceService.postHttpCall("getCategory", formData).subscribe((data: any) => {
  ...
  onSubmit() {
    ...
    this.requestServiceService.postHttpCall("updateCategroyImage", formData).subscribe((data: any) => {
      if (data.status === 'SUCCESS') {
        console.log(data);
